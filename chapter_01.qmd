---
abstract: |
  Money is not just a medium of exchange — it is an institution that solves a fundamental coordination problem. This chapter establishes the conceptual foundations for the entire course: what money is, how its supply is structured, how the price level is determined, and why the quality of monetary institutions shapes economic outcomes. The regression theorem explains why money cannot be created by decree from nothing, a constraint that bears directly on modern debates about cryptocurrencies and central bank digital currencies.
---
...
![](figures/chapter_01/functions_of_money.png){width="80%"}
...
![](figures/chapter_01/balance_composition.png)
...
![](figures/chapter_01/money_composition.png)
...
![](figures/chapter_01/money_aggregates.png){width="80%"}
...
![](figures/chapter_01/money_aggregates_values.png){width="80%"}
...
::: {#fig-barter}
![](figures/chapter_01/barter.png){width="35%"}
...
::: {#fig-regression-theorem}
```{mermaid}
flowchart LR
    A["🪙 Commodity Value\n(non-monetary origin)"]
    B["Early Monetary Use"]
    C["Established Money"]
    D["Gold Standard /\nBretton Woods"]
    E["Present Value"]
    ...
    D -->|"inherits value from predecessor"| F["Fiat Money\n(post-1971)"]
    D -->|"no commodity anchor —\nvalue from expectations alone?"| G["Bitcoin / Crypto"]

    style A fill:#B87333,color:#fff,stroke:#36454F
    style B fill:#5B9BD5,color:#fff,stroke:#36454F
    style C fill:#5B9BD5,color:#fff,stroke:#36454F
    style D fill:#5B9BD5,color:#fff,stroke:#36454F
    style E fill:#5B9BD5,color:#fff,stroke:#36454F
    style F fill:#87A96B,color:#fff,stroke:#36454F
    style G fill:#D4745E,color:#fff,stroke:#36454F
```

The regression theorem traces monetary value backward through time: each period's acceptance of money as payment is grounded in its value in the prior period, with the chain terminating at a non-monetary commodity origin. Fiat money extended the chain by inheriting value from the gold-linked money it replaced. Bitcoin and other cryptocurrencies face a bootstrapping problem — with no commodity anchor, initial monetary value must emerge from expectations alone.
:::
...
![](figures/chapter_01/history_of_money.png)
...
::: {#fig-inflation}
![](figures/chapter_01/inflation.png)
...
## From Money to Banking

The debate between private money and state money — which today takes the form of cryptocurrencies versus CBDCs — is not new. It has been running for centuries, and it has been resolved before, in different ways and with different results. To evaluate where it is heading now, we need to understand where it has already been.

Chapter 1 has given you the conceptual vocabulary: what money is, how its supply is structured, how the price level is determined, and why the quality of monetary institutions matters for economic outcomes. Chapter 2 uses that vocabulary to examine a specific historical question: how did banking actually work in the era *before* central banks, when private institutions competed to issue money under a commodity standard? The answer will challenge several things you probably think you know about financial history — and it will establish the baseline against which central bank performance should be judged.
