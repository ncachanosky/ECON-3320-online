---
abstract: |
  Every financial asset is a stream of future cash flows discounted to the present. This chapter applies that principle to bonds and stocks, developing the pricing formulas that practitioners use daily and the concepts — yield to maturity, duration, the yield curve, the Gordon Growth Model — that give those formulas analytical meaning. The chapter also examines the Efficient Market Hypothesis: not as a claim that markets are perfect, but as a claim about how quickly prices incorporate available information, with direct implications for how investors should behave.
---
...
::: {#fig-duration}
![](figures/chapter_06/duration.png)
...
![](figures/chapter_06/typical_yield_curves.png)
...
The yield curve plotted from standard coupon bonds — the Treasury yield curve you see quoted daily — is actually a composite. Each point on that curve is the yield to maturity of a coupon-paying bond, and YTM, as noted in §6.1, is an internal rate of return that averages across all of the bond's cash flows. A 10-year Treasury bond paying semi-annual coupons does not produce a clean measure of the market's required return for the 10-year horizon specifically — some of those coupons arrive in years 1, 2, 3, and so on, each implicitly discounted at rates appropriate for their own maturities. The YTM blends all of those discount rates together.

For many analytical purposes — pricing derivatives, valuing complex instruments, testing the expectations hypothesis precisely — economists and practitioners want a purer measure: the **spot rate** $s_T$, which is the market's required return for a single cash flow received exactly $T$ years from today, with no intervening payments. Spot rates eliminate the averaging problem entirely.

Zero-coupon bonds provide the cleanest path to spot rates. Because a zero-coupon bond pays nothing until maturity, its yield to maturity *is* the spot rate for that maturity — there are no intermediate cash flows to complicate the picture. The relationship is direct:

$$s_T = \left(\frac{F}{p}\right)^{1/T} - 1$$

where $F$ is the face value and $p$ is today's price. No averaging, no blending — just the single required return for the single horizon $T$. And notice that for a zero-coupon bond, duration equals maturity exactly ($D = T$), a property established in §6.1. This is not a coincidence: it reflects the same underlying logic. YTM equals the spot rate for zero-coupon bonds precisely *because* there is only one cash flow and therefore only one horizon to discount — the absence of intermediate payments makes duration and maturity collapse to the same number.

In practice, the US Treasury does not issue zero-coupon bonds directly, but the Treasury's [STRIPS](https://en.wikipedia.org/wiki/United_States_Treasury_security#STRIPS) program — introduced in Chapter 5 — creates them synthetically. Stripping a 10-year Treasury bond produces 21 separate zero-coupon instruments: 20 semi-annual coupon strips and one principal strip, each maturing on a different date. The market prices of these strips give direct observations of spot rates at each of those 21 maturities. Connecting them produces the **zero-coupon yield curve** (also called the **spot rate curve**), which is the term structure of interest rates in its purest form.

The par yield curve (constructed from coupon bonds trading at par) and the spot rate curve generally have similar shapes but differ in level — the spot rate curve is slightly steeper when the par curve is upward-sloping, because the longer maturities contain cash flows that are discounted at successively higher rates. When the par curve is inverted, the spot curve tends to invert more sharply. These differences matter when pricing instruments precisely, though for the broad intuitions developed in this course — reading the yield curve shape, understanding the expectations hypothesis, identifying inversions — the par curve and the spot curve tell the same story.
...
::: {#fig-country_risk}
![](figures/chapter_06/country_risk.png)
...
What does that risk premium look like in practice? The most comprehensive long-run evidence comes from [Dimson, Marsh, and Staunton](https://en.wikipedia.org/wiki/Equity_premium_puzzle), who assembled stock and bond return data across 35 countries going back to 1900. For the United States, the historical **equity risk premium** — the average annual return on equities above the return on government bills — has been approximately 5–7% per year depending on the measurement period and whether returns are measured arithmetically or geometrically. Other developed markets show similar but slightly lower premia, typically in the 3–6% range. This premium is not guaranteed: there have been decade-long stretches — most notably the 1970s and the 2000s — where US equities underperformed Treasury bonds significantly. But over horizons of 20 years or more, equities have historically rewarded investors for bearing the additional risk.

For a Gordon Growth Model application, this empirical anchor is useful. If the current 10-year Treasury yield is 4% (the risk-free rate) and the equity risk premium is roughly 5%, then the required return on a broad equity index is approximately 9%. That is the $i$ to plug into $p = D/(i-g)$. Whether the Gordon Growth Model's output is plausible depends heavily on whether these inputs — especially the long-run dividend growth rate $g$ — are realistic. The model's sensitivity to small input changes, noted above, is precisely why analysts treat the ERP estimate as a calibration anchor rather than a precise forecast.
...
::: {#fig-malkiel-monkey}
![](figures/chapter_06/malkiel_monkey.jpg)
